...
SCRIPT = [
    ...
    (  # 13 料金（仮・要相談）
     ["「料金は決め打ちしていません。学校のご予算事情はよく分かっていますので、"
      "規模とご予算に合わせて個別にお見積りします」",
      "「いただく費用はファイルの代金ではなく、エラー対応・毎年の年度更新の立ち会い・"
      "担当交代時の再レクチャー・様式が変わったときの改修、この4つの引き受け料です」",
      "金額の話はここで深追いしない。「まずトライアルで価値を確認してから、"
      "ご予算の枠を伺って提案させてください」で次のスライドへ"],
     None,
     "「予算がない」と言われたら：「トライアルは無料で、費用は一切かかりません。"
     "金額のご相談はその後で結構です」とだけ返す"),
    (  # 14 クロージング
