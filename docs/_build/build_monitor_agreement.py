# -*- coding: utf-8 -*-
"""導入協力校覚書（ひな形・有償の特別価格版）を生成する。

先行導入校への特別価格と協力事項を書面化するためのたたき台。
学校に提示する前に、必要に応じて専門家（行政書士等）の確認を推奨。

生成: docs/02_営業・商談資料/導入協力校覚書_ひな形.pdf
"""
import os
import subprocess
import tempfile
...
def build(docx_path):
    ...
    para(doc, "「積立金会計 入力アシスタント」導入協力校に関する覚書（案）",
         size=15, bold=True, color=NAVY, center=True, space_after=10)

    para(doc, "＿＿＿＿＿＿＿＿＿＿（以下「学校」という）と ＿＿＿＿＿＿＿＿＿＿（以下「提供者」という）は、"
              "積立金会計 入力アシスタント（以下「本製品」という）の先行導入について、以下のとおり合意する。",
         size=10.5, space_after=10)

    article(doc, "第1条（目的）", [
        "提供者は、学校に対し本製品を先行導入校としての特別価格で提供し、"
        "学校は本製品の実務における有用性の検証に協力する。",
    ])
    article(doc, "第2条（提供物と期間）", [
        "1. 提供物：本製品一式（Excelブック・練習用データ・各種手順書）、導入時の説明（訪問またはオンライン）、"
        "および期間中のサポート（メール・オンライン）。",
        "2. 契約期間：＿＿＿＿年＿＿月＿＿日から1年間とする。",
        "3. 利用料：年額 金＿＿＿＿＿円（税別）とする。これは先行導入校としての特別価格であり、"
        "第3条の協力事項が継続される限り、次年度以降も同額に据え置く。",
    ])
    ...
    article(doc, "第5条（期間満了時の取り扱い）", [
        "1. 契約期間の満了1ヶ月前までに、提供者は次年度の継続条件（第2条の据え置き価格）を提示し、"
        "学校は継続または終了を選択する。学校の意思確認なく自動更新はしない。",
...
if __name__ == "__main__":
    ...
    to_pdf(docx_path, os.path.join(here, "..", "02_営業・商談資料", "導入協力校覚書_ひな形.pdf"))
